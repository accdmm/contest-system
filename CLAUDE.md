...
| 模块 | 说明 | 已存在？ | 注意点 |
|------|------|---------|--------|
| **后端接口：会话列表** `GET /api/ai/conversations` | 返回当前用户所有会话 `id, title, updateTime`，按时间倒序 | ✅ 已实现 | 只返回元数据，不返回消息体；校验 userId 归属 |
| **后端接口：会话消息** `GET /api/ai/conversations/{id}/messages` | 返回指定会话的所有消息 `role, content` | ✅ 已实现 | 按 createTime 升序；校验归属 |
| **后端接口：删除会话** `DELETE /api/ai/conversations/{id}` | 删除会话及其所有消息 | ✅ 已实现 | 校验归属；手动删除关联 `ai_message` |
| **已有接口复用** `POST /api/ai/chat` | 传入 `conversationId=null` 自动创建新会话，传入已有 id 继续对话 | ✅ 已实现 | 前端无需改动 |
| **前端：会话侧边栏** | 浮动面板左侧展开，显示会话标题列表 + 新建按钮 | ✅ 已实现 | 小屏可折叠；保持 `AiBubble.vue` 原有的右下角气泡入口 |
| **前端：切换会话** | 点击会话条目加载历史消息 | ✅ 已实现 | 切换时清空消息列表后重新请求后端加载；避免内存泄漏 |
| **前端：删除会话** | 删除按钮 + 二次确认 | ✅ 已实现 | 先调用后端删除接口，再从列表中移除 |
| **前端：新建会话** | 新建按钮 → 创建空会话 → 用户可立即发消息 | ✅ 已实现 | 传 `conversationId=null` 给后端即可 |
